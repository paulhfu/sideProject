...
from loss import DiceLoss, CeLoss
...
if __name__ == '__main__':
    ...
    criterion = CeLoss(weights=torch.tensor(cfg.general.lossWeights, device=device))
    ...
    for dataIdx in cfg.general.dataIdx:
        ...
        # model = vgg16_bn()
        # model.load_state_dict(vgg_features)

        # if not accs:
        #     modelName = cfg.model.name
        # else:
        #     modelName = 'fs_model_Acc_'+str(best_acc)[0:5]+'_data'+str(max(accs))+'.pth'
        # hier auf jeden fall noch vgg freezen
        # criterion = DiceLoss(weights=cfg.general.lossWeights)
        if cfg.general.usePascalVOC:
