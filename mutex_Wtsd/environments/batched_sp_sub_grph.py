...
from utils.reward_functions import FocalReward, FullySupervisedReward, ObjectLevelReward, UnSupervisedReward, \
    GraphDiceReward, GlobalSparseReward, SubGraphDiceReward
...
class SpGcnEnv(Environment):
    ...
    def execute_action(self, actions, logg_vals=None, post_stats=False):
        ...
        reward = self.reward_function.get(last_diff, self.sg_current_edge_weights, self.get_current_soln()).to(self.device)
    ...
    def get_state(self):
        ...
        return state_pixels, self.b_edge_ids, self.sp_indices, self.b_edge_angles, self.b_subgraph_indices, self.sep_subgraphs, self.counter, self.b_gt_edge_weights
