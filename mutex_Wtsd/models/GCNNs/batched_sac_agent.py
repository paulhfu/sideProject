...
class GcnEdgeAC(torch.nn.Module):
    def __init__(self, cfg, args, device, writer=None):
        ...
        self.actor = PolicyNet(self.args.n_embedding_features * self.args.s_subgraph, self.args.s_subgraph * 2)
        self.critic = DoubleQValueNet((1 + self.args.n_embedding_features) * self.args.s_subgraph, n_q_vals)
        self.critic_tgt = DoubleQValueNet((1 + self.args.n_embedding_features) * self.args.s_subgraph, n_q_vals)
    ...
    def forward(self, raw, gt_edges=None, sp_indices=None, edge_index=None, angles=None, round_n=None,
        ...
        if actions is None:
            with torch.set_grad_enabled(policy_opt):
                out = self.actor(sg_edge_features.detach())
            ...
            q1, q2 = self.critic_tgt(torch.cat((sg_edge_features, actions), dim=-1))
            return dist, q1, q2, actions

        q1, q2 = self.critic(torch.cat((sg_edge_features, actions), dim=-1))
        return q1, q2


class PolicyNet(torch.nn.Module):
    def __init__(self, n_in_features, n_classes):
    ...
    def forward(self, edge_features):
        return self.stats(edge_features)
...
class DoubleQValueNet(torch.nn.Module):
    ...
    def forward(self, edge_features):
        return self.value1(edge_features), self.value2(edge_features)
