...
model = GcnEdgeConvNet3(n_node_features_in=3, n_edge_classes=1, device=device, softmax=False)
...
for epoch in range(1000):
    edges, edge_feat, gt_edge_weights, node_feat, seg, gt_seg, _, _ = next(iter(dloader_disc))
    node_feat, edge_feat, edges, gt_edge_weights = node_feat.squeeze(0), edge_feat.squeeze(0), edges.squeeze(0), gt_edge_weights.squeeze(0)
    ...
    out = model(node_feat, edge_feat, edges)
