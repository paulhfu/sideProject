...
class AgentOffpac(object):
    ...
    def _step(self, memory, shared_model, env, optimizer, off_policy=True, writer=None):
        ...
        for t in transition_data:
            if not t.terminal:
                _, q_, _ = self.agent_forward(env, self.shared_damped_model, t.state_, False)
            ...
            pvals_t = pvals.gather(-1, t.action.unsqueeze(-1)).squeeze()
            ...
            if t.terminal:
                ...
                c_loss = c_loss + nn.functional.mse_loss((t.reward + self.args.discount * q_.max(-1)[0]) * m, q_t * m)
        ...
        for i in batch_ind:
            ...
            a_loss = a_loss - (policy_proba_t.detach()/behav_policy_proba_t) * w * torch.log(policy_proba_t) * advantage_t
        z = z / len(batch_ind)
        a_loss = a_loss / z
