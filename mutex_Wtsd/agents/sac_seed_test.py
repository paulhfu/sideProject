...
from models.GCNNs.sac_agent3 import GcnEdgeAC_3
...
class AgentSacTrainer_test(object):
    ...
    def __init__(self, cfg, args, global_count, global_writer_loss_count, global_writer_quality_count,
        ...
        super(AgentSacTrainer_test, self).__init__()
    ...
    def agent_forward(self, env, model, state, actions=None, grad=True, post_input=False, policy_opt=False):
        with torch.set_grad_enabled(grad):
            state_pixels, edge_ids, sp_indices, edge_angles, counter, b_gt_edge_weights = self.state_to_cuda(state)
    ...
    def train_step(self, rank, start_time, return_dict, writer):
        ...
        model = GcnEdgeAC_3(self.cfg, self.args.n_raw_channels, self.args.n_embedding_features, 1, device, writer=writer)
    ...
    def state_to_cpu(self, state):
        state_pixels, edge_ids, sp_indices, edge_angles, counter, b_gt_edge_weights = state
        return state_pixels.cpu(), edge_ids.cpu(), sp_indices, edge_angles, counter, b_gt_edge_weights.cpu()

    def state_to_cuda(self, state):
        state_pixels, edge_ids, sp_indices, edge_angles, counter, b_gt_edge_weights = state
        return state_pixels.to(self.device), edge_ids.to(self.device), sp_indices, \
               edge_angles, counter, b_gt_edge_weights.to(self.device)