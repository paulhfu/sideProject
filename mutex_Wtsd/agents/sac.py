...
from models.GCNNs.sac_agent2 import GcnEdgeAC_2
...
class AgentSacTrainer(object):
    ...
    def train(self, rank, start_time, return_dict):
        ...
        model = GcnEdgeAC_2(self.cfg, self.args.n_raw_channels, self.args.n_embedding_features, 1, device, writer=writer)
                ...
                if self.cfg.MC_DQL:
                    ...
                    while not env.done:
                        ...
                        if self.global_count.value() < self.cfg.num_seed_steps:
                            ...
                            distr, _, _, action = self.agent_forward(env, shared_model, state=state_ep + [state],
                        ...
                        (_, _, _, _, next_state), reward, quality = env.execute_action(action, logg_dict)

                        if self.args.add_noise:
                            noise = torch.randn_like(reward) * self.alpha
                            reward = reward + noise
