...
from models.GCNNs.batched_sac_agent import GcnEdgeAC
...
class BatchedSgRewAgentSacTrainer(object):
    ...
    def __init__(self, cfg, args, global_count, global_writer_loss_count, global_writer_quality_count,
        ...
        super(BatchedSgRewAgentSacTrainer, self).__init__()
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, env, model, optimizers):
        ...
        target_Q1, target_Q2 = target_Q1.squeeze(), target_Q2.squeeze()
        log_prob = distribution.log_prob(next_action).sum(-1)
        # target_Q1, target_Q2 = self.critic_target(next_obs, next_action)
        target_V = torch.min(target_Q1, target_Q2) - self.alpha.detach() * log_prob
        target_Q = reward.sum(-1) + (not_done * self.cfg.discount * target_V)
    ...
    def update_actor_and_alpha(self, obs, env, model, optimizers):
        ...
        # action = dist.rsample()
        actor_Q1, actor_Q2 =actor_Q1.squeeze(), actor_Q2.squeeze()
