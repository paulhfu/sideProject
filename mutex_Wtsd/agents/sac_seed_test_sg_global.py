...
class AgentSacTrainer_test_sg_global(object):
    ...
    def state_to_cpu(self, state):
        state_pixels, edge_ids, sp_indices, edge_angles, sub_graphs, sep_subgraphs, counter, b_gt_edge_weights = state
        return state_pixels.cpu(), edge_ids.cpu(), sp_indices, edge_angles, sub_graphs.cpu(), sep_subgraphs.cpu(), counter, b_gt_edge_weights.cpu()

    def state_to_cuda(self, state):
        state_pixels, edge_ids, sp_indices, edge_angles, sub_graphs, sep_subgraphs, counter, b_gt_edge_weights = state
        return state_pixels.to(self.device), edge_ids.to(self.device), sp_indices, \
               edge_angles, sub_graphs.to(self.device), sep_subgraphs.to(self.device), counter, b_gt_edge_weights.to(self.device)
...
class SacValidate(object):
    ...
    def state_to_cuda(self, state):
        state_pixels, edge_ids, sp_indices, edge_angles, sub_graphs, sep_subgraphs, counter, b_gt_edge_weights = state
        return state_pixels.to(self.device), edge_ids.to(self.device), sp_indices, \
               edge_angles, sub_graphs.to(self.device), sep_subgraphs.to(
            self.device), counter, b_gt_edge_weights.to(self.device)